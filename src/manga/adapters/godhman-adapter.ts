...
@Injectable()
export class GodhmanAdapter extends BaseMangaAdapter implements MangaScraperAdapter {
  readonly websiteKey = 'godhman';
  readonly websiteName = 'Godhman';
  readonly websiteUrl = 'https://www.godhman.net';

  protected readonly logger = new Logger(GodhmanAdapter.name);
  ...
  async getLatestUpdated(page = 1, limit = 10): Promise<MangaItemDto[]> {
    ...
    try {
      ...
      const scrapedData = await this.scrapeMangaListWithPuppeteer(latestUrl, limit, {
        waitForSelector: '#content > div > div.postbody > div:nth-child(3) > div.listupd',
    ...
    return await page.evaluate(
      (url, maxLimit) => {
        try {
          ...
          const mangaSelectors = ['#content > div > div.postbody > div:nth-child(3) > div.listupd > div.bs'];
          ...
          mangaElements.forEach((element, index) => {
            ...
            try {
              ...
              const lastUpdated = document.querySelector('.epxdate')?.textContent?.trim();
              if (title && mangaId) {
                mangas.push({
                  id: mangaId,
                  title: title,
