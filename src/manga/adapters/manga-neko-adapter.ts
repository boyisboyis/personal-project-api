...
@Injectable()
export class MangaNekoAdapter extends BaseMangaAdapter implements MangaScraperAdapter {
  readonly websiteKey = 'manga-neko';
  readonly websiteName = 'Manga Neko';
  readonly websiteUrl = 'https://manga-neko.com';

  protected readonly logger = new Logger(MangaNekoAdapter.name);
  ...
  async getLatestUpdated(page = 1, limit = 10): Promise<MangaItemDto[]> {
    ...
    try {
      ...
      const scrapedData = await this.scrapeMangaListWithPuppeteer(latestUrl, limit, {
        waitForSelector: '#content > div > div.postbody > div:nth-child(2) > div.listupd',
    ...
    return await page.evaluate(
      (url, maxLimit) => {
        try {
          ...
          // Manga-neko.com specific selectors
          const mangaSelectors = ['#content > div > div.postbody > div:nth-child(2) > div.listupd > .bs'];
          ...
          mangaElements.forEach((element, index) => {
            ...
            try {
              ...
              if (!title) {
                ...
                if (linkElement) {
                  title = linkElement.textContent?.trim() || linkElement.getAttribute('title')?.trim() || `Unknown Manga ${index + 1}`;
              ...
              if (mangaUrl) {
                ...
                if (potentialId && potentialId !== 'manga') {
                  mangaId = potentialId;
                }
              }

              // Extract cover image
              const imageSelectors = ['.limit .ts-post-image'];
              let coverImage = '';
              for (const selector of imageSelectors) {
                const imgElement = element.querySelector(selector) as HTMLImageElement;
                if (imgElement) {
                  coverImage = imgElement.getAttribute('data-src') || imgElement.getAttribute('data-lazy-src') || '';
              ...
              const chapterSelectors = ['ul > li:nth-child(1) .fivchap'];
              ...
              const lastUpdated = element.querySelector('ul.chfiv > li:nth-child(1) > a > span.fivtime')?.textContent?.trim();
          ...
          console.log(`Extracted ${mangas.length} manga items from ${url}`);
          return mangas;
        } catch (error) {
  ...
  async extractMangaDetails(page: Page, url: string): Promise<MangaItemDto | null> {
    ...
    return await page.evaluate(mangaUrl => {
      try {
        // Extract basic info
        const titleSelectors = ['div.seriestucon > div.seriestuheader > h1.entry-title', '.entry-title'];
        let title = '';
        for (const selector of titleSelectors) {
        ...
        // Extract author
        const authorSelectors = ['div.seriestucontentr > div.seriestucont > div > table > tbody > tr:nth-child(5) > td:nth-child(2)'];
        let author = '';
        for (const selector of authorSelectors) {
          const element = document.querySelector(selector);
          if (element) {
            author = element.textContent?.trim().replace(/^(Author|Created by|By):?\s*/i, '') || '';
            if (author) break;
          }
        }

        // Extract cover image
        const imageSelectors = ['div.seriestucon > div.seriestucontent > div.seriestucontl > div.thumb > img'];
        let coverImage = '';
        for (const selector of imageSelectors) {
          ...
          if (chapterElements.length > 0) {
            chapterElements.forEach(element => {
              ...
              if (linkElement) {
                ...
                const chapterMatch = element.getAttribute('data-num');
                const chapterNumber = chapterMatch ? parseFloat(chapterMatch) : chapters.length + 1;
                ...
                if (chapterUrl) {
                  ...
                  const potentialId = urlParts[urlParts.length - 1] || urlParts[urlParts.length - 2];
                  if (potentialId && potentialId !== 'chapter') {
                    chapterId = potentialId;
                  }
                }

                // Extract date if available
                const dateElement = element.querySelector('.chapterdate');
  ...
  async extractChapterImages(page: Page, chapterUrl: string): Promise<ChapterImageDto[]> {
    return await page.evaluate(url => {
      try {
        ...
        // Manga-neko.com specific selectors for chapter images
        const imageSelectors = ['#readerarea > img'];
  ...
  private generateMockLatestUpdated(limit: number): MangaItemDto[] {
    return Array.from({ length: limit }, (_, i) => ({
      id: `manga-neko-${i + 1}`,
      title: `Manga Neko Title ${i + 1}`,
      author: 'Manga Neko Author',
      coverImage: `https://via.placeholder.com/200x300?text=MangaNeko+${i + 1}`,
      latestChapter: i + 15,
      lastUpdated: new Date().toISOString(),
      url: `${this.websiteUrl}/manga/manga-neko-${i + 1}`,
      chapters: [],
    }));
  }
  ...
  private generateMockMangaDetails(mangaKey: string): MangaItemDto {
    return {
      id: mangaKey,
      title: 'Manga Neko Demo Title',
      author: 'Manga Neko Demo Author',
      coverImage: 'https://via.placeholder.com/200x300?text=MangaNeko+Demo',
      latestChapter: 30,
      lastUpdated: new Date().toISOString(),
